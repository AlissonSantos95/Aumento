...
def CreateWindow():
    ...
    lbl = Button(window, text="login", command=lambda: window.destroy() & os.system("Login1.py"))#botao login
    lbl.place(x=245, y=430)# localização do botão "login"

    lbl1 = Button(window, text="Sair", command=window.destroy)#botao login
